...
int		SocketManager::open_listening_sockets(Kqueue *kq) {
	for (size_t i = 0; i < listening.size(); ++i) {
		...
		kq->kqueue_add_event(listening[i]->get_listening_connection());	// kqueue에 등록
	}
	return WEBSERV_OK;
}
...
void	SocketManager::close_listening_sockets() {
	for(size_t i = 0; i < listening.size(); ++i) {
		...
		if (close_socket(ls->get_fd()) == -1) {
			std::cerr << "close socket error..." << std::endl;
		}
		delete ls;
	}
}
...
Connection*		SocketManager::get_connection(socket_t s) {
	...
	c = free_connections;
	if (c == NULL) {
		std::cerr << "worker_connections are not enough..." << std::endl;
		return NULL;
	}
